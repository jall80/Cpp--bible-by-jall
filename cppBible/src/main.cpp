...
enum class Arrow { None = 0, Up = 1, Down = 2, Both = 3, Error = -1 };

///////////////TOOLS///////////////////////////////////////////////////////////////////////////////////////////////////////////////

// Función para verificar un solo campo
template <typename U> bool verifyField(const U &field) {
...
bool verifyStructValues(const T &s, Members T::*...members) {
  // Usamos un "fold expression" para aplicar la verificación a todos los
  // miembros
  return (... && verifyStructMember(
                     s, members)); // Aplica la verificación para cada miembro
}

/**
 * @brief Checks if a container is empty and throws an exception if it is.
 *
 * @tparam Container The type of the container (e.g., std::vector, std::list,
 * std::set).
...
void validateNotEmpty(const Container &container, const std::string &varName,
                      const std::string &funcName) {
  if (container.empty()) {
    throw std::invalid_argument("Validation failed in function \"" + funcName +
                                "\": The container \"" + varName +
                                "\" cannot be empty.");
  }
}

// Macro to simplify calling the function with the variable name
#define VALIDATE_NOT_EMPTY(container, funcName)                                \
  validateNotEmpty(container, #container, funcName)

/**
 * @brief Checks if a value is below a specified limit and throws an exception
 * if it is.
 *
 ...
 */
template <typename T>
void validateNotBelowLimit(const T &value, const T &limit,
  ...
  if (value < limit) {
    throw std::invalid_argument("Validation failed in function \"" + funcName +
                                ...
                                std::to_string(value) +
                                " cannot be less than \"" + limitName +
                                "\" -> " + std::to_string(limit) + ".");
  }
}

// Macro to simplify calling the function with the variable name
#define VALIDATE_NOT_BELOW_LIMIT(value, limit)                                 \
  validateNotBelowLimit(value, limit, #value, #limit, __func__)

// Macro para obtener el nombre del vector
#define VALIDATE_VECTOR(vector, funcName)                                      \
  validateLineTextParams(vector, #vector, funcName)
 ...
 * in the vector meet expected criteria (e.g., positive values, non-empty
 * strings, valid ranges).
 *
 ...
 */
void validateLineTextParams(
    const std::vector<LineTextParams> &paramsList,
  ...
  for (size_t idx = 0; idx < paramsList.size(); ++idx) {
    ...
    std::string errorMessage = funcName + " -> ( Vector: " + vectorName +
                               ", index: " + std::to_string(idx + 1) + ")";
...
void verifyScreenParams(const ScreenParams &screenParams) {
  ...
  if (!verifyStructValues(screenParams, &ScreenParams::width,
                          &ScreenParams::height, &ScreenParams::leftMargin,
                          &ScreenParams::rightMargin, &ScreenParams::topMargin,
                          &ScreenParams::bottomMargin)) {
    throw std::invalid_argument(
        "Error: in " + std::string(__func__) +
        ", the 'screenParams' struct contains invalid values");
  }
  ...
  if (screenParams.leftMargin > screenParams.width / 6 ||
    ...
    throw std::invalid_argument(
        "Error: in " + std::string(__func__) +
        ", margins must be at least 1/6 of the width and height");
  }
}

/**
 * @brief Recursively enumerates topics and their subtopics, adding a numbered
 * format based on depth.
 *
 * @param topics A reference to a vector of LineTextParams objects representing
 * the topics to be enumerated. Each LineTextParams contains a `text` field and
 * an optional pointer to its subtopics.
 * @param depth (Optional) The current depth level of enumeration. Defaults to 0
 * for top-level topics. Determines the indentation of the numbering.
 */
void enumerateTopics(std::vector<LineTextParams> &topics, int depth = 0) {
  ...
  for (size_t i = 0; i < topics.size(); ++i) {
    ...
    numberedText << std::string(depth * 2, ' ') << (i + 1) << ". "
                 << topics[i].text;
...
/**
 * @brief Filters topics based on their width, removing those that exceed the
 * maximum width allowed for the screen. If a topic exceeds the allowed width,
 * an error is thrown.
 *
 * @param topics A reference to a vector of LineTextParams representing the
 * topics to be checked. Each LineTextParams contains a `text` field and an
 * optional pointer to its subtopics.
 * @param screenParams A reference to the ScreenParams structure, which contains
 * information about the screen dimensions and margins used to calculate the
 * maximum allowed width for the topics.
 *
 * @throws std::runtime_error If a topic exceeds the maximum width, an error is
 * thrown with a descriptive message.
 */
void filterTopicsBySize(std::vector<LineTextParams> &topics,
  ...
  const uint16_t maxWidth =
      screenParams.width - general4BlocksX.blockX2 - screenParams.leftMargin;

  // Lambda function to check if a topic fits within the screen width
  auto fitsWithinScreen = [&](const LineTextParams &topic) {
    // Estimate the width of the topic text in pixels based on font size and
    // font resize factor
    uint16_t estimatedWidth =
        topic.text.size() * (topic.fontSize * 0.9) * FONT_RESIZE;
  ...
  topics.erase(std::remove_if(topics.begin(), topics.end(),
                              [&](LineTextParams &topic) {
                                // Recursively filter subtopics to ensure their
                                // size also fits within the screen
                                if (topic.subtopics) {
                                  filterTopicsBySize(*topic.subtopics,
                                                     screenParams);
                                }

                                // If the topic doesn't fit within the allowed
                                // width, throw an error
                                if (!fitsWithinScreen(topic)) {
                                  throw std::runtime_error(
                                      "Topic '" + topic.name +
                                      "' exceeds the maximum width of " +
                                      std::to_string(maxWidth) + " pixels.");
                                }

                                // No topics are deleted here, only validated
                                // for size
                                return false;
                              }),
               topics.end());
 ...
 *        The stack supports basic stack operations: push, pop, peek, and check
 * if it's empty.
 */
class Stack {
private:
  std::vector<int16_t>
      stack; ///< The internal vector that holds the stack elements.
...
public:
  ...
  void push(int16_t value) {
    stack.push_back(
        value); ///< Adds the value to the stack (end of the vector).
  }
  ...
  int16_t pop() {
    if (!stack.empty()) {
      ...
      std::cerr << "Error: Pila vacía."
                << std::endl; ///< Error message if stack is empty.
 ...
 * @brief A class that controls a thread, allowing it to be started, paused,
 * resumed, and stopped. It provides thread management with synchronization
 * mechanisms like mutexes and condition variables.
 */
class ThreadController {
...
private:
  ...
  std::mutex
      controlMutex; ///< Mutex used to synchronize access to control flags.
  std::condition_variable pauseCondition; ///< Condition variable to handle
                                          ///< pausing and resuming the thread.
  std::atomic<bool>
      stopFlag; ///< Atomic flag to indicate if the thread should stop.
  std::atomic<bool>
      pauseFlag; ///< Atomic flag to indicate if the thread is paused.
...
public:
      ...
      while (!stopFlag) {
        // Handle pause: wait if the thread is in a paused state
        {
          ...
          pauseCondition.wait(lock,
                              [this]() { return !pauseFlag || stopFlag; });
        }
  ...
  void pause() {
    ...
    pauseCondition
        .notify_all(); ///< Notify the thread to check the pause condition.
  }
  ...
  void resume() {
    ...
    pauseCondition
        .notify_all(); ///< Notify the thread to check the resume condition.
  }
  ...
  void stop() {
    ...
    pauseCondition
        .notify_all(); ///< Notify the thread to check the stop condition.
  ...
  /**
   * @brief Destructor that ensures the thread is stopped before the object is
   * destroyed.
   */
  ~ThreadController() {
 ...
 *        It includes basic attributes such as name, width, height, position,
 * and velocity.
 */
class BaseObject {
protected:
...
public:
   ...
   */
  BaseObject(const std::string &_name, uint16_t _width, uint16_t _height,
             uint16_t _x_position, uint16_t _y_position)
      : name(_name), width(_width), height(_height), x_position(_x_position),
        y_position(_y_position), velocity_x(0.0f), velocity_y(0.0f) {}
   ...
   */
  BaseObject(const std::string &_name, uint16_t _width, uint16_t _height,
             uint16_t _x_position, uint16_t _y_position, float _velocity_x,
             float _velocity_y)
      : name(_name), width(_width), height(_height), x_position(_x_position),
        y_position(_y_position), velocity_x(_velocity_x),
...
/**
 * @class TextureObject
 * @brief A derived class from BaseObject that adds texture handling
 * functionality to the object. This class uses the SFML library to load and
 * display textures for graphical representation.
 */
class TextureObject : public BaseObject {
private:
  sf::Texture texture; ///< Texture for the object (SFML Texture).
  sf::Sprite
      textureSprite; ///< Sprite to render the texture on screen (SFML Sprite).
...
public:
   ...
   * @param _y_position The Y-coordinate position of the object.
   */
  TextureObject(const std::string &_name, uint16_t _width, uint16_t _height,
                uint16_t _x_position, uint16_t _y_position)
      : BaseObject(_name, _width, _height, _x_position, _y_position) {}

  /**
   * @brief Constructor to initialize the texture object with velocity.
   *        Inherits from BaseObject and initializes the texture object with
   * velocity.
   *
   * @param _name The name of the object.
   * @param _width The width of the object.
   ...
   *        Adjusts the sprite's position and scale based on the object
   * dimensions.
   *
  ...
  bool loadTexture(const std::string &path) {
    ...
    textureSprite.setPosition(
        static_cast<float>(x_position),
        static_cast<float>(y_position)); // Set sprite position.
    textureSprite.setScale(
        static_cast<float>(width) /
            texture.getSize().x, // Scale sprite to fit object width.
        static_cast<float>(height) /
            texture.getSize().y // Scale sprite to fit object height.
    );
    return true; // Return true if texture loading and sprite setup are
                 // successful.
  }
  ...
  const sf::Sprite &getTextureSprite() const {
    return textureSprite; // Return the texture sprite object as a constant
                          // reference.
  }
  ...
  void updatePosition() {
    textureSprite.setPosition(
        static_cast<float>(x_position),
        static_cast<float>(y_position)); // Set the sprite's position.
  }
...
/**
 * @class TextObject
 * @brief Represents a text object for rendering text on a screen with potential
 * child and parent relationships. This class supports text alignment, color,
 * font management, and hierarchical rendering.
 *
 * @param _name       The name of the object.
 ...
 * @param _centerText Boolean flag to specify if the text should be
 * center-aligned.
 *
 * @return None
 */
...
private:
  ...
  std::vector<std::shared_ptr<TextObject>>
      children;                     // List of child text objects
  std::weak_ptr<TextObject> parent; // Weak pointer to the parent text object

public:
  /**
   * @brief Constructor for initializing a TextObject with a specified position,
   * color, text, font, and size.
   *
   ...
   *
   * @throw std::runtime_error If the font cannot be loaded from the given font
   * path.
   * @return None
   */
  TextObject(const std::string &_name, uint16_t _x_position,
             uint16_t _y_position, std::array<uint16_t, 3> _color,
             const std::string &_text, const std::string &_fontPath,
             uint16_t _text_size, bool _centerText)
      : BaseObject(_name, 0, 0, _x_position, _y_position), color(_color),
        fontPath(_fontPath), centerText(_centerText) {
    ...
    textObj.setPosition(static_cast<float>(_x_position),
                        static_cast<float>(_y_position));
  }
   ...
   *
   * @return A shared pointer to the parent TextObject, or an empty shared
   * pointer if no parent exists.
   */
   ...
   *
   * @return A reference to a vector containing shared pointers to child
   * TextObjects.
   */
   ...
   * @return None
   */
  void draw(sf::RenderWindow &window) { window.draw(textObj); }

  /**
   * @brief Prints the hierarchy of the TextObject and its children for
   * debugging purposes.
   *
  ...
  void updateTextPosition() {
    textObj.setPosition(static_cast<float>(x_position),
                        static_cast<float>(y_position));
  }

  /**
   * @brief Sets the font size of the text object and optionally applies it to
   * all children.
   *
   * @param newFontSize The new font size to set.
   * @param applyToChildren Whether to apply the new font size to all child
   * TextObjects.
   *
   * @return None
   */
   ...
   */
  std::array<uint16_t, 3> getColor() const { return color; }
   ...
   */
  std::string getText() const { return textObj.getString(); }
   ...
   */
  uint16_t getTextSize() const { return textObj.getCharacterSize(); }
   ...
   */
  bool isCentered() const { return centerText; }
   ...
   */
  float getTextWidth() const { return textObj.getLocalBounds().width; }
   ...
   */
  float getTextHeight() const { return textObj.getLocalBounds().height; }
 ...
 *
 * @param generalTopics A vector containing the general topics at the first
 * level.
 * @param indices A vector of indices used to traverse through the levels of
 * subtopics.
 *
 * @return A vector of `LineTextParams` containing the subtopics at the deepest
 * level of the hierarchy. If any level doesn't have subtopics or the indices
 * are invalid, the function returns an empty vector.
 *
 * @throws std::out_of_range If an index is out of bounds at any level.
 */
std::vector<LineTextParams>
findNestedSubtopics(const std::vector<LineTextParams> &generalTopics,
  ...
  for (int index : indices) {
    ...
    if (index < 0 || index >= currentLevel->size()) {
      throw std::out_of_range(
          "Index out of range in generalTopics or subtopics.");
    }
 ...
 * and if the distance is smaller than the specified minimum distance
 * (`minDistance`), it returns `false`, indicating that the point is too close
 * to one of the objects. If the point is far enough from all objects, the
 ...
 * @param minDistance The minimum required distance between the point and each
 ...
 */
bool isFarEnough(const std::vector<TextureObject *> &objects, float x, float y,
                 float minDistance) {
  // Validate that x, y, and minDistance are positive
  if (x <= 0 || y <= 0 || minDistance <= 0) {
    throw std::invalid_argument(
        "x, y, and minDistance must be positive values.");
  }

  // Iterate over each object in the list
  for (const auto &obj : objects) {
    // Calculate the difference in x and y coordinates between the point and the
    // object
 ...
 *
 * This function creates a specified number (`n`) of moving objects with random
 * sizes and positions within a given grid. It ensures that the objects are
 * placed far enough from each other to avoid overlap. The size of each object
 * is chosen randomly within a specified range, and the object positions are
 * checked to ensure they do not violate the minimum distance requirement from
 * other objects.
 *
 * @param n The number of moving objects to create. Must be between 0 and 10.
 * @param grid A 2D grid represented as a vector of vectors of `Block` objects,
 * used to define the area in which the objects can be placed.
 * @param object The `StructTextureObject` containing the properties of the
 * object to be created, such as its size, velocity, and texture path.
 *
 * @return A vector of pointers to dynamically allocated `TextureObject`
 * instances. If the creation fails at any point, an empty vector is returned.
 *
 * @throws std::runtime_error If the number of objects is greater than 10, or if
 * the size of the object does not meet the minimum size requirements.
 */
std::vector<TextureObject *>
createMovingObjects(int n, std::vector<std::vector<Block>> grid,
                    StructTextureObject object) {
  // Check if the number of objects is valid
  if (n <= 0)
    return {}; // Return an empty vector if n is less than or equal to 0

  if (n > MAX_MOVING_OBJS) {
    throw std::invalid_argument("Max num of moving objects is " +
                                std::to_string(MAX_MOVING_OBJS) +
                                ". Got: " + std::to_string(n));
  }

  // Ensure the object size meets the minimum size requirement
  if (MIN_SIZE * 2 > object.width) {
    throw std::invalid_argument("The size of the square object must be at "
                                "least double the minimum size. "
                                "Got: " +
                                std::to_string(object.width) +
                                ", required: " + std::to_string(MIN_SIZE * 2));
  }
  ...
  std::vector<TextureObject *>
      movingObjectPointer; // Vector to store pointers to moving objects
  ...
  for (int i = 0; i < n; ++i) {
    ...
    int size =
        ...
        (std::rand() %
         (object.width -
          (MIN_SIZE - 1))); // Random size between MIN_SIZE and object.width
    float posX, posY;
    ...
    while (!validPosition) {
      ...
      posX =
          grid[8][16].x + (std::rand() % (grid[100][60].x - grid[8][16].x + 1));
      posY =
          grid[8][16].y + (std::rand() % (grid[100][60].y - grid[8][16].y + 1));
    ...
    std::unique_ptr<TextureObject> texture =
        std::make_unique<TextureObject>(object.name, size, size, posX, posY,
 ...
 *
 * This function iterates through a list of `TextureObject` pointers and draws
 * each valid object onto the provided `sf::RenderWindow`. If an object pointer
 * is `nullptr`, it skips that object to avoid any potential errors.
 *
 * @param objects A vector of pointers to `TextureObject` instances, which
 * represent the objects to be drawn on the window.
 * @param window The `sf::RenderWindow` instance where the objects will be
 * drawn.
 *
 * @return void
 */
void drawObjects(const std::vector<TextureObject *> &objects,
 ...
 *
 * This function checks if the sound button is clicked. If the button is clicked
 * and music is currently playing, it pauses the music and changes the button's
 * texture to the "sound-off" icon. If the music is paused, it starts the music
 * and changes the button's texture to the "sound-on" icon. The function also
 * uses atomic operations to handle the `isMusicPlaying` state safely in a
 * multi-threaded environment.
 *
 * @param soundButton A reference to the `sf::Sprite` that represents the sound
 * button.
 * @param soundOnTexture A reference to the `sf::Texture` that represents the
 * sound-on icon.
 * @param soundOffTexture A reference to the `sf::Texture` that represents the
 * sound-off icon.
 * @param music A reference to the `sf::Music` object that controls the
 * background music.
 * @param isMusicPlaying An atomic boolean to track whether the music is
 * currently playing.
 * @param window A constant reference to the `sf::RenderWindow` where the button
 * is located.
 *
 * @return void
 */
void handleSoundButton(sf::Sprite &soundButton, sf::Texture &soundOnTexture,
                       sf::Texture &soundOffTexture, sf::Music &music,
  ...
  if (soundButton.getGlobalBounds().contains(
...
/**
 * @brief Handles the interaction with the moving button to toggle object
 * movement.
 *
 * This function checks if the moving button is clicked. If the button is
 * clicked and the thread controller is paused, it clears the current moving
 * objects, creates new ones using the `createMovingObjects` function, and
 * resumes the thread. If the thread is not paused, it clears the current moving
 * objects, sets the button to the "off" texture, and pauses the thread. This
 * ensures that the objects' movement can be controlled by the button.
 *
 * @param movingButton A reference to the `sf::Sprite` that represents the
 * moving button.
 * @param movingONTexture A reference to the `sf::Texture` representing the
 * "moving-on" button texture.
 * @param movingOffTexture A reference to the `sf::Texture` representing the
 * "moving-off" button texture.
 * @param window A constant reference to the `sf::RenderWindow` used to check
 * the mouse position.
 * @param threadController A reference to the `ThreadController` responsible for
 * pausing and resuming the thread.
 * @param movingObjectPointer A reference to the vector of `TextureObject*`
 * containing the moving objects.
 * @param grid A reference to the 2D grid used to place the moving objects.
 * @param object A reference to the `StructTextureObject` containing the
 * properties of the moving objects.
 *
 * @return void
 */
void handleMovingButton(sf::Sprite &movingButton, sf::Texture &movingONTexture,
  ...
  if (movingButton.getGlobalBounds().contains(
          static_cast<sf::Vector2f>(mousePos))) {
    ...
    if (threadController.isPaused()) {
      ...
      movingButton.setTexture(
          movingONTexture);      // Set the "on" texture for the button
      ...
      movingButton.setTexture(
          movingOffTexture);    // Set the "off" texture for the button
 ...
 *
 * This function attempts to load music from a file located at the provided
 * `music_path`. If the music file is successfully loaded, it is played and set
 * to loop. If the loading fails, an error message is displayed and `nullptr` is
 * returned to indicate failure.
 *
 * @param music_path The relative path to the music file in AUDIOS_PATH.
 *
 * @return A `std::shared_ptr<sf::Music>` pointing to the loaded and playing
 * music, or `nullptr` if the loading failed.
 */
 ...
 *
 * This function iterates over a vector of integer pointers, where each pointer
 * points to an array. It searches for a specified `number` in each array,
 * stopping when the number is found or the end of the array is reached
 * (indicated by `-1`). If the number is found, the function returns the first
 * element of the array where the number is located. If the number is not found
 * in any array, the function returns `-1`.
 *
 * @param vec The vector containing pointers to integer arrays.
 * @param number The number to search for in the arrays.
 *
 * @return The first element of the array where the number is found, or `-1` if
 * the number is not found in any array.
 */
int findFirstArrayWithNumber(const std::vector<int *> &vec, int number) {
  ...
  for (size_t i = 0; i < vec.size(); i++) {
    ...
    for (size_t j = 0; array[j] != -1; j++) {
      if (array[j] == number) {
        return array[0]; // Return the first element of the array where the
                         // number is found
      }
    }
  }
...
/**
 * @brief Determines the relative position of an array containing a specific
 * number in a vector of arrays.
 *
 * This function searches through a vector of integer arrays for a given number.
 * Upon finding the number, it checks if the array has a preceding (up) or
 * succeeding (down) array in the vector. The function returns an `Arrow` value
 * that indicates the relation between the found array and its surrounding
 * arrays:
 ...
 * @return Arrow Enumeration value indicating the relation (Up, Down, Both, or
 * Error).
 */
  ...
  for (size_t i = 0; i < vec.size(); i++) {
    ...
    for (size_t j = 0; array[j] != -1; j++) {
      if (array[j] == number) {
        ...
        bool hasNext =
            (i < vec.size() - 1); // There is a next array if i < vec.size() - 1
 ...
 *
 * This function takes the relation result returned by
 * `findArraySurroundingRelation` and draws arrows accordingly. It uses the
 * `Arrow` enumeration to determine which arrows to display:
 ...
 *
 * @param relationResult The result of the relation check (from
 * `findArraySurroundingRelation`).
 ...
 */
void drawArrowsBasedOnRelation(
    Arrow relationResult, sf::RenderWindow &window,
    const std::unique_ptr<TextureObject> &upArrow,
    const std::unique_ptr<TextureObject> &downArrow) {
  ...
  if (relationResult == Arrow::Up) {
    ...
    std::cerr << "Error: Invalid relationResult value. "
                 "findArraySurroundingRelation() returned error. Value: "
 ...
 *
 * This function takes a `std::vector<int>` of indices and creates a dynamic
 * array of integers to hold the same values. The size of the dynamic array
 * matches the size of the input vector, and the values from the vector are
 * copied into the array.
 *
 * @param indices The vector of integers that will be copied into the dynamic
 * array.
 * @return A pointer to the dynamically allocated array of integers, or nullptr
 * if allocation fails.
 */
int *createIndexArray(const std::vector<int> &indices) {
  // Create a dynamic array to store the indices
  int *indexArray =
      new int[indices.size()]; // Dynamically allocate memory for the array

  // Copy the values from the vector into the dynamic array
  std::copy(indices.begin(), indices.end(),
            indexArray); // Copy elements from the vector to the array
 ...
 *
 * This function calculates whether the text objects exceed the available screen
 * space (taking margins into account). If the next line of text exceeds the
 * screen's bottom margin, a new array of indices is created and stored in the
 * result vector. It also handles cases where the remaining elements fit within
 * the screen.
 *
 ...
 * @return A vector of dynamically allocated arrays containing indices of text
 * objects in different screen segments.
 */
std::vector<int *>
predictTextOverflow(const std::shared_ptr<TextObject> &rootNode,
                    const ScreenParams &screen, sf::RenderWindow &window) {
  const auto &children =
      rootNode->getChildren(); // Get the child text objects of the root node
  std::vector<int *> overflowIndices; // Vector to store arrays of indices for
                                      // overflow segments
  std::vector<int>
      currentIndices; // Temporary list of indices before overflow occurs
  int next_line = LINE_SPACING * 2 + screen.topMargin +
                  50; // Initialize the position for the next line
  ...
  for (size_t i = 0; i < children.size(); i++) {
    ...
    if (next_line > screen.height - screen.bottomMargin) {
      ...
      next_line = LINE_SPACING * 2 + screen.topMargin +
                  50; // Reset the next line position
    }

    // Add the current node's index to the list
    currentIndices.push_back(i);

    // Update the next line position based on the height of the current text
    // node
    next_line += children[i]->getTextHeight() + LINE_SPACING;
  }

  // Handle any remaining elements that didn't cause overflow (final segment
  // without overflow)
...
/**
 * @brief Draws the nodes starting from the root node, handling overflow and
 * screen layout.
 *
 * This function iterates through the root node and its children, positioning
 * them on the screen. It starts by drawing the root node, then draws each child
 * node below it, taking into account the screen's height and margin. If the
 * screen's space is insufficient (overflow), the drawing stops.
 *
 ...
 * @param overflow The index to start drawing children from, used for handling
 * overflow.
 */
void drawMenuFromRoot(const std::shared_ptr<TextObject> &rootNode,
                      const ScreenParams &screen, sf::RenderWindow &window,
                      int overflow) {
  ...
  uint16_t next_line =
      screen.topMargin + 50; // Start drawing from the top margin

  // Draw the root node centered on the screen
  uint16_t root_x_position =
      general4BlocksX.blockX2; // Adjust to center on screen
  rootNode->setXPosition(
      root_x_position);              // Set the X position of the root node
  rootNode->setYPosition(next_line); // Set the Y position of the root node
  rootNode
      ->updateTextPosition(); // Update the text position for correct rendering
  rootNode->draw(window);     // Draw the root node

  next_line += LINE_SPACING * 2; // Add space below the root node for children

  uint16_t child_x_position =
      general4BlocksX.blockX2; // Set the X position for child nodes
  ...
  for (size_t i = overflow; i < children.size(); ++i) {
    ...
    children[i]->setYPosition(
        next_line); // Set the Y position for the child node
    children[i]->setXPosition(
        child_x_position);             // Set the X position for the child node
    children[i]->updateTextPosition(); // Update the text position for correct
                                       // rendering
 ...
 *
 * This function adds a list of subtopics to the root node by associating each
 * subtopic as a child of the root node. It also sets the root node as the
 * parent of each subtopic, establishing the parent-child relationship.
 *
 * @param root A shared pointer to the root node, which will be populated with
 * subtopics.
 * @param subTopics A vector of shared pointers to subtopics to be added to the
 * root node.
 */
...
/**
 * @brief Creates a list of `TextObject` instances based on the provided topic
 * data.
 *
 * This function takes a vector of `LineTextParams` (referred to as `topics`)
 * and creates a corresponding list of `TextObject` instances. Each `TextObject`
 * is initialized using the properties from the `LineTextParams` and added to
 * the vector to be returned.
 *
 * @param topics A vector of `LineTextParams` structures, each containing
 * parameters for creating a `TextObject`.
 *
 * @return A vector of shared pointers to the created `TextObject` instances.
 */
std::vector<std::shared_ptr<TextObject>>
createTextObjects(const std::vector<LineTextParams> &topics) {
  std::vector<std::shared_ptr<TextObject>>
      textObjects; // Vector to store the created text objects
...
/**
 * @brief Recursively creates a tree structure of `TextObject` nodes from
 * provided text parameters.
 *
 * This function builds a hierarchical tree of `TextObject` nodes starting from
 * a root node, with each node potentially having child nodes (subtopics). It
 * stops the recursion either when the maximum depth is reached or when there
 * are no more subtopics to process.
 *
 * @param root A shared pointer to the root `TextObject`, which will act as the
 * starting point of the tree.
 * @param nodes A vector of `LineTextParams`, each representing the parameters
 * for creating a `TextObject`.
 * @param maxDepth The maximum depth of the tree. The function will stop
 * recursively creating nodes if this depth is reached.
 * @param currentDepth The current depth of the tree during recursion. Defaults
 * to 1.
 */
void createTree(
    const std::shared_ptr<TextObject>
        &root, // Root node to start creating the tree
    const std::vector<LineTextParams>
        &nodes,          // List of nodes (with parameters) to create from
    int maxDepth,        // Maximum depth for the recursion
    int currentDepth = 1 // Current depth of the recursion (default to 1)
) {
  // Base case: If the root is null or the maximum depth is reached, stop the
  // recursion.
  ...
  // (nodes).
  auto subTopics = createTextObjects(
      nodes); // Create a vector of TextObjects from the node parameters.

  // Step 2: Populate the root node with these created children (subtopics).
  populateRootWithSubtopics(
      root, subTopics); // Add the child nodes (subTopics) to the root node.

  // Step 3: Recursively process each node's subtopics if they exist and the
  // maximum depth is not exceeded.
  for (size_t i = 0; i < nodes.size(); ++i) {
    const auto &node = nodes[i];

    // Check if the current node has subtopics (children). If so, process them
    // recursively.
    if (node.subtopics.has_value()) {
      // Recursively create a tree of subtopics for the current node.
      createTree(subTopics[i], node.subtopics.value(), maxDepth,
                 currentDepth + 1);
    }
  }
}
...
/**
 * @brief Determines if two `TextureObject` instances are colliding based on
 * their positions and dimensions.
 *
 * This function checks whether the bounding boxes of two objects overlap,
 * indicating a collision. The overlap detection can be fine-tuned using the
 * `OVERLAP_PERCENTAGE` factor, which adjusts the effective size of the bounding
 * boxes considered during the collision check.
 *
...
bool isColliding(const TextureObject *obj1, const TextureObject *obj2) {
  ...
  return !(obj1->getXPosition() + obj1->getWidth() * OVERLAP_PERCENTAGE <
               obj2->getXPosition() ||
           obj1->getXPosition() >
               obj2->getXPosition() + obj2->getWidth() * OVERLAP_PERCENTAGE ||
           obj1->getYPosition() + obj1->getHeight() * OVERLAP_PERCENTAGE <
               obj2->getYPosition() ||
           obj1->getYPosition() >
               obj2->getYPosition() + obj2->getHeight() * OVERLAP_PERCENTAGE);
}

/**
 * @brief Moves multiple `TextureObject` instances within a window, managing
 * velocity, boundary collisions, and inter-object collisions.
 *
 * This function operates in a loop to continuously update the positions of
 * objects, check for collisions with the window boundaries and other objects,
 * and adjust their velocities accordingly. It supports pause functionality
 ...
 * @param needsRedraw An atomic flag to signal that the objects need to be
 * redrawn.
 * @param isPaused An atomic flag indicating whether the movement is paused.
 */
void moveObjects(std::vector<TextureObject *> &objects,
                 const sf::RenderWindow &window, std::atomic<bool> &needsRedraw,
  ...
  try { // Using try logic because this isn't critical
    ...
    while (!isPaused.load() && window.isOpen()) {
      ...
      for (size_t i = 0; i < objects.size(); ++i) {
        ...
        if (posX <= EXTRA_MARGING ||
            posX + obj->getWidth() >= window.getSize().x - EXTRA_MARGING) {
          obj->setVelocityX(-velocity_x); // Reverse X velocity
        }
        if (posY <= EXTRA_MARGING ||
            posY + obj->getHeight() >= window.getSize().y - EXTRA_MARGING) {
...
/**
 * @brief Retrieves a const reference to a unique_ptr<TextureObject> from the
 * vector based on its name.
 *
 * This function iterates through a vector of TextureObject instances managed by
 * unique_ptr. It compares the name of each object with the provided name and
 * returns a const reference to the matching unique_ptr if found. If no match is
 * found, it throws an exception or can return a null reference.
 *
 * @param textureObjects A vector of unique_ptr<TextureObject> containing the
 * objects to search through.
 * @param name The name of the TextureObject to retrieve.
 * @return const std::unique_ptr<TextureObject>& A const reference to the
 * matching unique_ptr, or throws an exception if not found.
 */
const std::unique_ptr<TextureObject> &getTextureObjectByName(
    const std::vector<std::unique_ptr<TextureObject>> &textureObjects,
  ...
  for (const auto &textureObject : textureObjects) {
    ...
    if (textureObject && textureObject->getName() == name) {
      return textureObject; // Return a const reference to the matching
                            // unique_ptr
    }
  }
  throw std::runtime_error("TextureObject not found: " +
                           name); // Handle not found case (optional)
}

/**
 * @brief Loads textures into a vector of unique_ptr<TextureObject>.
 *
 * This function iterates through a vector of texture information structures
 * (StructTextureObject), creates TextureObject instances for each, attempts to
 * load the associated texture, and stores the successfully created
 * TextureObjects into a provided vector.
 *
 * @param textureObjects A vector of unique_ptr<TextureObject> to store loaded
 * textures.
 * @param textureInfos A vector of StructTextureObject containing texture
 * metadata.
 * @return int Returns 0 if all textures are loaded successfully, or -1 if any
 * texture fails to load.
 */

int loadTextures(std::vector<std::unique_ptr<TextureObject>> &textureObjects,
...
uint16_t initAndStartMainWindowLoop() {
  ...
  sf::RenderWindow window(
      sf::VideoMode(generalScreen.width, generalScreen.height), WINDOW_TITLE);
  ...
  auto mainMenu = std::make_shared<TextObject>(
      mainMenuParams.name, mainMenuParams.x_position, mainMenuParams.y_position,
      mainMenuParams.color, mainMenuParams.text, mainMenuParams.fontPath,
      mainMenuParams.fontSize, mainMenuParams.centered);
  ...
  std::vector<int *> overflow_list =
      predictTextOverflow(selectedMenu, generalScreen, window);

  std::atomic<bool> needsRedraw(true);

  // puedo destruirlos y volverlos a contruir
  std::vector<TextureObject *> movingObjectPointer =
      createMovingObjects(8, grid, iconCpp150);

  ThreadController movementThread;

  movementThread.start([&]() {
    moveObjects(std::ref(movingObjectPointer), std::cref(window),
                std::ref(needsRedraw), std::ref(movementThread.getPauseFlag()));
  });
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      ...
      case sf::Event::KeyPressed:
        ...
        if (event.key.code == sf::Keyboard::Escape) {
          ...
          if (!subtopics.empty()) {
            ...
            last_types.push(
                types); // save last position in a stack to come back later
            types = 0;  // Bringing back user to first element of the new
                        // deployed list
            selectedMenuChildren[types]->setFontSize(BIG_FONT);
            overflow_list =
                predictTextOverflow(selectedMenu, generalScreen, window);
      ...
      case sf::Event::MouseButtonPressed:
        if (event.mouseButton.button == sf::Mouse::Left) {
          handleSoundButton(
              getTextureObjectByName(textureObjects, SOUND_ON_OBJ)
                  ->getTextureSprite(),
              getTextureObjectByName(textureObjects, SOUND_ON_OBJ)
                  ->getTexture(),
              getTextureObjectByName(textureObjects, SOUND_OFF_OBJ)
                  ->getTexture(),
              *music, isMusicPlaying, window);
          handleMovingButton(
              getTextureObjectByName(textureObjects, CPP100FILL_IMAGE_OBJ)
                  ->getTextureSprite(),
              getTextureObjectByName(textureObjects, CPP100FILL_IMAGE_OBJ)
                  ->getTexture(),
              getTextureObjectByName(textureObjects, X100_IMAGE_OBJ)
                  ->getTexture(),
              window, movementThread, movingObjectPointer, grid, iconCpp150);
          needsRedraw = true;
        }
        break;
    ...
    if (needsRedraw) {
      window.clear();
      getTextureObjectByName(textureObjects, BACKGROUND_IMAGE_OBJ)
          ->draw(window);
      ...
      getTextureObjectByName(textureObjects, CPP100FILL_IMAGE_OBJ)
          ->draw(window);
      drawMenuFromRoot(selectedMenu, generalScreen, window,
                       findFirstArrayWithNumber(overflow_list, types));
      Arrow temp = findArraySurroundingRelation(overflow_list, types);
      drawArrowsBasedOnRelation(
          temp, window,
          getTextureObjectByName(textureObjects, UP_ARROW_IMAGE_OBJ),
          getTextureObjectByName(textureObjects, DOWN_ARROW_IMAGE_OBJ));
      window.display();
      needsRedraw = false;
    } else {
